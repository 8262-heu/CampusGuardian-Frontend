...
function loadStudentOverview(){
  ...
  fetch(`${BASE_API}/by-email?email=${encodeURIComponent(email)}`)
    ...
    .then(list => {
      ...
      if(list.length === 0){
        body.innerHTML = `<tr><td colspan="5">No complaints yet.</td></tr>`;
        return;
      }
...
function submitComplaintStudent(){
  ...
  const dName = deptSelect.options[deptSelect.selectedIndex]?.text || "";
  const msg = document.getElementById("submitMsg");

  if(n==="" || e==="" || m==="" || dId===""){
    msg.style.color="orange";
    msg.innerText="Fill all fields including department";
    return;
  }
  ...
  fetch(BASE_API,{
    ...
    body: JSON.stringify({
      ...
      status: "Pending",
      departmentId: dId,
      departmentName: dName   // ✅ THIS WAS MISSING
    })
  })
  .then(res=>{
    if(res.ok){
